...
public class SbdConsumer extends BaseConsumer {
	...
	public CompletableFuture<ResponseMessage> createOutgoingDefault(StandardBusinessDocumentHeader sbdh,
			PapyrosDocument papDoc, Authorization auth) throws JAXBException {
		...
		return httpClient.preparePost(sendEndpoind).addHeader("Content-Type", "application/xml")
				.addHeader("Authorization", authorizationHeader).setBody(outputStream.toByteArray()).execute()
	...
	// ResponseMessage rm = new ResponseMessage();
	// try {
	// String authHeader = auth.getUsername().toString() + ":" +
	// auth.getPassword().toString();
	// String authHeaderEncoded =
	// Base64.getEncoder().encodeToString(authHeader.getBytes("utf-8"));
	// authorizationHeader = "Basic " + authHeaderEncoded;
	// } catch (UnsupportedEncodingException e) {
	// throw new EDeliveryException(e);
	// }
	// return httpClient.prepareGet(messageUri).addHeader("Authorization",
	...
	// */
	// return StreamUtils.stream2String(resp.getResponseBodyAsStream(),
	// "UTF-8");
	// } catch (Exception ex) {
	// Logger.getLogger(SbdConsumer.class.getName()).log(Level.SEVERE, null,
	// ex);
	// }
	// }
	// break;
	...
	public CompletableFuture<Messages> getMesaggesPending(Authorization auth) {
		...
		return httpClient.prepareGet(messagesAll).addHeader("Authorization", authorizationHeader).execute()
				.toCompletableFuture().exceptionally(t -> {
					throw new EDeliveryException(t);
				}).thenApply(resp -> {
					int status = resp.getStatusCode();
